...
public class DownFalling : MonoBehaviour {
	...
	void Start () {
		...
		thisScript = SpawnTimer.scriptDownControllTwo;
	...
	void Update () {	
		...
		if ( failed == false){
			...
			if (Input.GetKeyDown(KeyCode.DownArrow) && inPlace == true){
				Debug.Log ("Points!!!");
		...
		if (Input.GetKeyDown(KeyCode.DownArrow) && inPlace == false && SpawnTimer.scriptDownControll == thisScript){
			Debug.Log ("fail!!!");
			failed = true;
