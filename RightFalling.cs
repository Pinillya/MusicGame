...
public class RightFalling : MonoBehaviour {
	...
	void Start () {
		...
		thisScript = SpawnTimer.scriptRightControllTwo;
	...
	void Update () {	
		...
		if ( failed == false){
			...
			if (Input.GetKeyDown(KeyCode.RightArrow) && inPlace == true){
				Debug.Log ("Points!!!");
		...
		if (Input.GetKeyDown(KeyCode.RightArrow) && inPlace == false && SpawnTimer.scriptRightControll == thisScript){
			Debug.Log ("fail!!!");
			failed = true;
		...
		if (addTimer > 0.1){
			Debug.Log ("timer Up");	
			Destroy (gameObject);
			SpawnTimer.scriptRightControll ++;
		}
