...
public class UpFalling : MonoBehaviour {
	...
	void Start () {
		...
		thisScript = SpawnTimer.scriptUpControllTwo;
	...
	void Update () {	
		...
		if ( failed == false){
			...
			if (Input.GetKeyDown(KeyCode.UpArrow) && inPlace == true){
				Debug.Log ("Points!!!");
		...
		if (Input.GetKeyDown(KeyCode.UpArrow) && inPlace == false && SpawnTimer.scriptUpControll == thisScript){
			Debug.Log ("fail!!!");
			failed = true;
